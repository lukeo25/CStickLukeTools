...
var iframe = document.createElement("iframe"); // FIX
            ...
            iframe.srcdoc = "<!doctype html><html><head><meta charset=\"utf-8\"><title>Clip Settings<\/title><style>*{box-sizing:border-box;}html,body{margin:0;padding:0;background:#1c1c22;color:#eaeaea;font-family:Arial,sans-serif;height:100%;overflow:hidden;}#app{display:flex;flex-direction:column;height:100%;}.topbar{flex:0 0 auto;display:flex;align-items:center;gap:8px;padding:8px 10px;border-bottom:1px solid rgba(255,255,255,0.12);background:rgba(15,15,16,0.96);}.title{font-weight:800;font-size:13px;flex:1;}.sel-pill{font-size:11px;padding:2px 8px;border-radius:4px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}.body{flex:1 1 auto;overflow-y:auto;overflow-x:hidden;padding:10px;}.row{margin-bottom:10px;}.row-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;}.lbl{font-size:12px;font-weight:700;}.badge{font-size:12px;font-weight:700;color:#7dd3a8;min-width:40px;text-align:right;}input[type=range]{width:100%;height:6px;accent-color:#21b26b;cursor:pointer;}select{width:100%;background:#1f1f22;color:#eaeaea;border:1px solid rgba(255,255,255,0.14);border-radius:6px;padding:6px 8px;font-size:12px;}input[type=text]{width:100%;background:#1f1f22;color:#eaeaea;border:1px solid rgba(255,255,255,0.14);border-radius:6px;padding:6px 8px;font-size:12px;}.footer{flex:0 0 auto;display:flex;gap:8px;padding:8px 10px;border-top:1px solid rgba(255,255,255,0.12);}.btn{flex:1;background:#1f1f22;color:#eaeaea;border:1px solid rgba(255,255,255,0.14);border-radius:6px;padding:7px;cursor:pointer;font-size:12px;font-weight:700;}.btn:hover{background:#27272b;}.btn-apply{background:#1a7a4a;border-color:#1a7a4a;}.btn-apply:hover{background:#218f56;}.status{font-size:11px;padding:0 10px 6px;min-height:16px;}.empty{padding:16px 10px;font-size:12px;color:#666;}<\/style><\/head><body><div id=\"app\"><div class=\"topbar\"><div class=\"title\">Clip Settings<\/div><div id=\"sel\" class=\"sel-pill\">No selection<\/div><\/div><div class=\"body\" id=\"body\"><div class=\"empty\" id=\"empty\">Select a clip with a script<\/div><\/div><div class=\"status\" id=\"status\"><\/div><div class=\"footer\"><button class=\"btn\" id=\"btnRefresh\">Refresh<\/button><button class=\"btn btn-apply\" id=\"btnApply\">Apply<\/button><button class=\"btn\" id=\"btnClose\">Close<\/button><\/div><\/div><script>var BR=null;function getB(){try{if(window.top&&window.top.LukeToolsBridge)return window.top.LukeToolsBridge;}catch(e1){}try{if(window.parent&&window.parent.LukeToolsBridge)return window.parent.LukeToolsBridge;}catch(e2){}return null;}function setStatus(m,ok){var s=document.getElementById('status');s.textContent=m||'';s.style.color=ok===false?'#f88':'#aaa';}var scriptSrc='';var controls=[];function parseCodeToControls(code){if(!code||!code.trim())return null;var src=code.trim();var cfgRx=/var\\s+([A-Z_][A-Z0-9_]*(?:_?CFG|_CONFIG|_PARAMS|CFG))\\s*=\\s*(\\{[^}]+\\})/;var cfgMatch=src.match(cfgRx);if(cfgMatch){var cfgVarName=cfgMatch[1];var cfgJsonStr=cfgMatch[2];var cfgObj=null;try{cfgObj=JSON.parse(cfgJsonStr);}catch(e){try{var r=cfgJsonStr.replace(/([{,]\\s*)([a-zA-Z_][a-zA-Z0-9_]*)(\\s*:)/g,'$1\\\"$2\\\"$3');cfgObj=JSON.parse(r);}catch(e2){}}if(cfgObj&&typeof cfgObj==='object'){var ctrls=[];for(var k in cfgObj){if(!cfgObj.hasOwnProperty(k))continue;if(k==='type')continue;var v=cfgObj[k];var t=typeof v;if(t==='number'){var isF=(String(v).indexOf('.')!==-1);var autoMax=Math.max(10,Math.abs(v)*10);var absV=Math.abs(v);var mn=0,mx=absV<=1?2:absV*3,st=isF?0.01:1;if(k==='phase'){mn=-6.28;mx=6.28;st=0.01;}else if(k==='amp'||k==='amplitude'){mn=0;mx=Math.max(absV*3,500);st=1;}else if(k==='speed'||k==='rate'){mn=0;mx=Math.max(absV*3,10);st=isF?0.01:1;}else if(k==='opacity'||k==='alpha'){mn=0;mx=1;st=0.01;}else if(k==='rotation'||k==='angle'){mn=0;mx=360;st=1;}else if(k==='gravity'){mn=0;mx=Math.max(absV*3,5);st=0.01;}else if(k==='jump'){mn=0;mx=Math.max(absV*3,20);st=0.1;}else if(k==='maxFall'||k==='maxfall'){mn=0;mx=Math.max(absV*3,30);st=0.1;}else{mn=v<0?-(absV*3):0;mx=absV<=1?2:absV*3;}ctrls.push({type:'number',key:k,label:k,val:v,min:mn,max:mx,step:st,cfgVar:cfgVarName});}else if(t==='boolean'){ctrls.push({type:'select',key:k,label:k,val:String(v),options:[{v:'true',l:'on'},{v:'false',l:'off'}],cfgVar:cfgVarName});}else if(t==='string'){ctrls.push({type:'text',key:k,label:k,val:v,cfgVar:cfgVarName});}}if(ctrls.length)return{controls:ctrls,cfgMode:true,cfgVar:cfgVarName,cfgJsonStr:cfgJsonStr};}}
// Also detect plain var LT_XXX = value (not starting with _LT_)
var ltRe=/(?:^|\n)\s*var\s+(LT_[A-Z0-9][A-Z0-9_]*)\s*=\s*([^;\n{[]+)/g;
var ltm;var ltCtrls=[];
while((ltm=ltRe.exec(src))!==null){
var ltName=ltm[1];var ltRaw=ltm[2].trim().replace(/\/\/.*$/,'').trim().replace(/;$/,'').trim();
var ltType='string';var ltVal=ltRaw;
if(ltVal==='true'||ltVal==='false'){ltType='boolean';}
else if(!isNaN(Number(ltVal))&&ltVal!==''){ltType='number';}
else{ltVal=ltVal.replace(/^["'`]|["'`]$/g,'');}
var absV2=Math.abs(Number(ltVal));
var mn2=0,mx2=absV2<=1?2:absV2*3,st2=(String(ltVal).indexOf('.')!==-1)?0.01:1;
ltCtrls.push({type:ltType,key:ltName,label:ltName,val:ltVal,min:mn2,max:mx2,step:st2,isLTVar:true});
}
if(ltCtrls.length)return{controls:ltCtrls,cfgMode:false};
return null;}function buildUI(parsed){var body=document.getElementById('body');body.innerHTML='';var empty=document.createElement('div');empty.id='empty';empty.className='empty';if(!parsed||!parsed.controls||!parsed.controls.length){empty.textContent='No CFG variables found';body.appendChild(empty);controls=[];setStatus('No editable variables found',false);return;}empty.style.display='none';body.appendChild(empty);controls=parsed.controls;controls.forEach(function(c,i){var row=document.createElement('div');row.className='row';var top=document.createElement('div');top.className='row-top';var lbl=document.createElement('div');lbl.className='lbl';lbl.textContent=c.label;top.appendChild(lbl);if(c.type==='number'){var badge=document.createElement('div');badge.className='badge';var isF=(String(c.step).indexOf('.')!==-1);badge.textContent=isF?Number(c.val).toFixed(2):String(c.val);top.appendChild(badge);row.appendChild(top);var slider=document.createElement('input');slider.type='range';slider.min=String(c.min);slider.max=String(c.max);slider.step=String(c.step);slider.value=String(c.val);slider.onmousedown=function(ev){ev.stopPropagation();};(function(ci,s,b,fl){s.oninput=function(){var v=parseFloat(s.value);if(!isFinite(v))v=0;controls[ci].val=v;b.textContent=fl?v.toFixed(2):String(Math.round(v));};s.setAttribute('data-idx',ci);})(i,slider,badge,isF);row.appendChild(slider);}else if(c.type==='select'){row.appendChild(top);var sel=document.createElement('select');sel.onmousedown=function(ev){ev.stopPropagation();};(c.options||[]).forEach(function(o){var opt=document.createElement('option');opt.value=o.v;opt.textContent=o.l;if(o.v===String(c.val))opt.selected=true;sel.appendChild(opt);});(function(ci,s){s.onchange=function(){controls[ci].val=s.value;};})(i,sel);row.appendChild(sel);}else{row.appendChild(top);var inp=document.createElement('input');inp.type='text';inp.value=String(c.val);inp.onmousedown=function(ev){ev.stopPropagation();};(function(ci,s){s.oninput=function(){controls[ci].val=s.value;};})(i,inp);row.appendChild(inp);}body.appendChild(row);});}function loadData(name,src){document.getElementById('sel').textContent=name||'Unknown';if(!src){setStatus('No update script found',false);return;}scriptSrc=src;var parsed=parseCodeToControls(src);buildUI(parsed);if(parsed&&parsed.controls)setStatus('Found '+parsed.controls.length+' variable'+(parsed.controls.length===1?'':'s'));}window.addEventListener('message',function(e){if(e.data&&e.data.type==='LT_CLIP_DATA'){loadData(e.data.name,e.data.script);}});function refresh(){var br=getB();if(!br){setStatus('Bridge not found - use auto-load',false);return;}try{if(br.captureSelectionNow)br.captureSelectionNow();}catch(e){}var name='';try{name=br.getSelectedClipName?br.getSelectedClipName():'';}catch(e){}if(!name){setStatus('No selection - reopen panel',false);return;}document.getElementById('sel').textContent=name;var src='';try{src=br.getUpdateScriptOnSelection?br.getUpdateScriptOnSelection():'';}catch(e){src='';}if(!src){setStatus('No update script found',false);return;}scriptSrc=src;var parsed=parseCodeToControls(src);buildUI(parsed);if(parsed&&parsed.controls)setStatus('Found '+parsed.controls.length+' variable'+(parsed.controls.length===1?'':'s'));}function applyVars(){var br=getB();if(!br){setStatus('Bridge not found',false);return;}if(!scriptSrc){setStatus('No script loaded',false);return;}if(!controls||!controls.length){setStatus('Nothing to apply',false);return;}var updated=scriptSrc;var c0=controls[0];if(c0&&c0.cfgVar){var cfgRx=new RegExp('(var\\\\s+'+c0.cfgVar+'\\\\s*=\\\\s*)(\\\\{[^}]+\\\\})');var newObj={};controls.forEach(function(c){var v=c.val;if(c.type==='number')v=parseFloat(v);else if(c.type==='select')v=(v==='true')?true:(v==='false')?false:v;newObj[c.key]=v;});try{var orig=scriptSrc.match(/var\\s+[A-Z_][A-Z0-9_]*(?:_CFG|_CONFIG|_PARAMS)\\s*=\\s*(\\{[^}]+\\})/);if(orig){var origObj=null;try{origObj=JSON.parse(orig[1]);}catch(e){}if(origObj&&origObj.type)newObj.type=origObj.type;}updated=updated.replace(cfgRx,function(m,prefix){return prefix+JSON.stringify(newObj)+';'.replace(';','');});}catch(e){setStatus('Replace failed: '+e.message,false);return;}}try{if(br.setUpdateScriptOnSelection)br.setUpdateScriptOnSelection(updated);scriptSrc=updated;setStatus('Applied!');}catch(e){setStatus('Failed: '+e.message,false);}}document.getElementById('btnRefresh').addEventListener('click',refresh);document.getElementById('btnApply').addEventListener('click',applyVars);document.getElementById('btnClose').addEventListener('click',function(){try{var br=getB();if(br&&br.closeGameSpritePanel)br.closeGameSpritePanel();}catch(e){};});refresh();<\/script><\/body><\/html>"; // FIX
